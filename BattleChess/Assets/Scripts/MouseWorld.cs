...
public class MouseWorld : MonoBehaviour
{
    ...
    private static Vector3 PastPos = Vector3.zero;
    private void Awake()
    {
        if (instance == null)
            instance = this;
    ...
    //static声明函数内部变量和方法使属于这个类而不是类的实例，允许不在脚本中实例化也能用.符号来访问内部变量
    {
        ...
        {
            if (Physics.Raycast(ray, out RaycastHit raycastHit, float.MaxValue, instance.MousePositionLayerMask)) 
            {
                instance.Mouse.SetActive(true);
                Vector3 targetP = raycastHit.transform.position;
                targetP.y = 0.6f;
                PastPos = targetP;
                return targetP;
            }
